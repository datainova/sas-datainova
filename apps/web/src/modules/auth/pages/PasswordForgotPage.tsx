...
import { requestPasswordResetLink } from '../../../core/auth/auth-api';
...
const schema = z.object({
  email: z
    .string({ required_error: 'Informe o email cadastrado.' })
...
const PasswordForgotPage = () => {
  ...
  const resetMutation = useMutation({
    mutationFn: async (email: string) => requestPasswordResetLink(email),
    onSuccess: (message) => {
      setFeedback(message);
    },
    onError: (error) => {
            ...
            <p
              className={`text-sm ${
                resetMutation.isError
                  ? 'text-danger-500'
                  : 'text-[rgba(45,41,38,0.7)] dark:text-[rgba(230,224,220,0.75)]'
              }`}
              role="alert"
            >
              {feedback}
